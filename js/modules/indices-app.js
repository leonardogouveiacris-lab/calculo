(function(global){
  ...
  function renderTables(){
    ...
    CPIndices.listTables().forEach(function(t){
      ...
      tr.innerHTML = '<td>' + t.id + '</td><td>' + t.nome + '</td><td>' + t.tipo + '</td><td>' + t.periodicidade + '</td><td>v' + t.versao + '</td><td>' + (t.updated_at || '-') + '</td><td><button data-action="edit" data-id="' + t.id + '">Editar</button> <button data-action="exp" data-id="' + t.id + '">Exportar</button></td>';
  ...
  function onAddSerie(){
    ...
    try {
      CPIndices.saveTable({ id: tableId, nome: $('tableNome').value || tableId, tipo: $('tableTipo').value || 'misto', fonte: $('tableFonte').value || '', periodicidade: $('tablePeriodicidade').value || 'mensal', updated_by: $('tableUpdatedBy').value || 'ui-local', serie: [{ competencia: competencia, valor: valor }] }, { upsert: true });
  ...
  function onImport(){
    ...
    file.text().then(function(text){
      ...
      $('importReport').textContent = rep.errors.length ? rep.errors.join('\n') : 'Sem inconsistências.';
      renderTables();
