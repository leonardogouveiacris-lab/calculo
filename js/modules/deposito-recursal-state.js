(function(global){
  ...
  function createModule(userConfig){
    ...
    function addMonths(monthKey, step){ const [y,m] = monthKey.split('-').map(Number); const dt = new Date(y, m - 1 + step, 1); return dt.getFullYear() + '-' + String(dt.getMonth()+1).padStart(2,'0'); }
    ...
    return { config, state, el, initElements, getEl, fmtBRL, toDateBR, toBR, monthKeyFromDate, addMonths, monthsBetweenInclusive, compareMonth, sortDeposits, minDepositDate, parseBCBNumber, monthLabel, esc, DEFAULT_HEADER, DEFAULT_FOOTER };
  }
